// backend/controller/manualEditController.js
 ...
 * @route   PUT /api/answers/:submissionId/edit-text/:questionNumber
 * @access  Private
 */
exports.editExtractedText = async (req, res) => {
  try {
    ...
    answer.grading.adjustmentReason = adjustmentReason || 'Manual adjustment by teacher';

    // Recalculate overall stats
    submission.calculateOverallStats();
    await submission.save();

    console.log(`📝 Score adjusted for Q${questionNumber}: ${originalScore} → ${newScore}`);
 ...
 * @route   PUT /api/answers/:submissionId/bulk-edit
 * @access  Private
 */
      ...
      try {
        ...
        if (editedText !== undefined && editedText !== null) {
          ...
          if (geminiApiKey && answer.status === 'graded') {
            const keyAnswer = answerKey.find(k => k.questionNumber === parseInt(questionNumber));
            if (keyAnswer) {
              try {
                await regradeAnswer(answer, keyAnswer, geminiApiKey);
                wasRegraded = true;
              } catch (regradeError) {
                console.error(`Regrade error for Q${questionNumber}:`, regradeError);
              }
            }
          }
        }
        ...
        if (newScore !== undefined && newScore !== null && answer.grading?.aiScore) {
          const maxScore = answer.grading.aiScore.maxScore;
          if (newScore >= 0 && newScore <= maxScore) {
            answer.grading.finalScore = newScore;
            answer.grading.isManuallyAdjusted = true;
 ...
 * @route   PUT /api/answers/:submissionId/reset/:questionNumber
 * @access  Private
 */
 ...
 * @route   GET /api/answers/:submissionId/edit-history
 * @access  Private
 */
    ...
    if (autoRegrade && answer.status === 'graded') {
      try {
        ...
        if (geminiApiKey) {
          ...
          if (keyAnswer) {
            console.log(`🔄 Auto-regrading question ${questionNumber} after text edit...`);
            ...
            console.log(`✅ Question ${questionNumber} re-graded: ${gradingResult.score}/${gradingResult.maxScore}`);
          }
        }
      } catch (regradeError) {
 ...
 * @route   PUT /api/answers/:submissionId/adjust-score/:questionNumber
 * @access  Private
 */
