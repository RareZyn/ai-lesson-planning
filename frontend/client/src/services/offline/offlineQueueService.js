...
import { addItem, getByIndex, updateItem, deleteItem, getAllItems, getItem } from '../indexedDB';
import api from '../api';
...
export async function queueAction(actionType, payload, metadata = {}) {
  try {
    const action = {
      // Note: 'id' will be auto-generated by IndexedDB (auto-increment)
      actionType,
      payload,
      metadata,
      status: ACTION_STATUS.PENDING,
      timestamp: new Date().toISOString(), // For index
      createdAt: new Date().toISOString(),
      attemptCount: 0,
      retryCount: 0, // For index
      maxAttempts: metadata.maxAttempts || 3,
      lastError: null,
    };

    const actionId = await addItem('offlineQueue', action);
    console.log(`✅ Action queued: ${actionType}`, { ...action, id: actionId });

    // Dispatch custom event for UI updates
    window.dispatchEvent(new CustomEvent('offline-action-queued', {
      detail: { action: { ...action, id: actionId } }
    }));
...
export async function updateActionStatus(actionId, status, error = null) {
  try {
    // Get the action by its primary key 'id'
    const action = await getItem('offlineQueue', actionId);
    ...
    if (action) {
      ...
      if (status === ACTION_STATUS.FAILED || status === ACTION_STATUS.RETRY) {
        updatedAction.attemptCount = (action.attemptCount || 0) + 1;
        updatedAction.retryCount = (action.retryCount || 0) + 1;
      }
  ...
  try {
    ...
    for (const action of pendingActions) {
      try {
        ...
        await updateActionStatus(action.id, ACTION_STATUS.PROCESSING);

        // Process the action
        const result = await processAction(action);

        // Mark as completed
        await updateActionStatus(action.id, ACTION_STATUS.COMPLETED);
        ...
        if (attemptCount < action.maxAttempts) {
          await updateActionStatus(action.id, ACTION_STATUS.RETRY, error);
          results.actions.push({
            actionId: action.id,
            type: action.actionType,
            status: 'retry',
          ...
          await updateActionStatus(action.id, ACTION_STATUS.FAILED, error);
          results.failed++;
          results.actions.push({
            actionId: action.id,
            type: action.actionType,
...
export async function retryFailedActions() {
  try {
    ...
    for (const action of actionsToRetry) {
      try {
        // Reset to pending
        await updateActionStatus(action.id, ACTION_STATUS.PENDING);
