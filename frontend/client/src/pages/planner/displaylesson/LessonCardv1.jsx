...
import styles from "./LessonCardv1.module.css";

const LessonCardv1 = ({ lesson, isRecent = false, assessments = [] }) => {
  ...
  const classDisplay =
    grade && className !== "Unknown Class" ? className : className;
    ...
    >
        ...
        {lesson.lessonDate && (
          <div className={styles.dateBadge}>
            <div className={styles.dayOfWeek}>
              {getDayOfWeek(lesson.lessonDate)}
            </div>
            <div className={styles.dateValue}>
              {getShortDate(lesson.lessonDate)}
            </div>
          </div>
        )}
      ...
      <div className={styles.cardContent}>
        ...
        {isRecent && lesson.updatedAt ? (
          ...
          <p className={styles.cardMeta}>Scheduled: {formatDate(lessonDate)}</p>
        )}
      </div>
    </div>
  );
